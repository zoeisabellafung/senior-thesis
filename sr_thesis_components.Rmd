...
postfire_whole <- rbind(
  ...
  select(-c(uv1, uv2, uv3, tag_no, status, dbh, ht, crwn_rto, tree_area)) %>%
  na.omit()
```
...
lm_class_plot <- ggplot(lm_fuel_class, aes(fill=fuel_class, y=value, x=plot_id)) + 
...
# stacked bar plot for fuel classes
bm_class_plot <- ggplot(bm_fuel_class, aes(fill=fuel_class, y=value, x=plot_id)) + 
  geom_bar(position="stack", stat="identity") +
...
lm_duff_plot <- ggplot(lm_duff_litter, aes(fill=fuel_class, y=value, x=plot_id)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("Fuel loading distribution at Bluff Mesa") +
  xlab("Plot") +
  ylab("Fuel loading (tons/acre)")
...
# stacked barplot for duff and litter
bm_duff_plot <- ggplot(bm_duff_litter, aes(fill=fuel_class, y=value, x=plot_id)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("Fuel loading distribution at Bluff Mesa") +
  xlab("Plot") +
  ylab("Fuel loading (tons/acre)")
...
lm_char_scatter <- laguna_cwm_arrange %>%
  ...
  na.omit() %>%
  mutate(char_ht=as.numeric(char_ht)) %>%
  mutate(location=rep("Laguna Mountain", 140))
...
summary(lr_sla_char) # review the results --> not significant
...
summary(lr_ll_char) # review the results --> not significant
...
summary(lr_lw_char) # review the results --> not significant
...
summary(lr_height_char) # review the results --> not significant
...
summary(lr_lc_char) # review the results --> not significant

# SLA v. scorch height
sla_scorch <- scorch_scatter %>%
  filter(trait=="Spec leaf area (mm2/mg)")
lr_sla_scorch = lm(scorch_ht~cwm, data = sla_scorch) # create the linear regression
summary(lr_sla_scorch) # review the results --> not significant
...
summary(lr_ll_scorch) # review the results --> not significant
...
summary(lr_lw_scorch) # review the results --> not significant
...
summary(lr_height_scorch) # review the results --> not significant
...
summary(lr_lc_scorch) # review the results --> not significant
...
```{r}
# char height
char_mult <- lm(mean_char_ht ~ sla_cwm + ll_cwm + width_cwm + height_cwm + curl_cwm, data = mean_char_ht) # multiple regression model
summary(char_mult) # display results of mult. regression

# scorch height
scorch_mult <- lm(mean_scorch_ht ~ sla_cwm + ll_cwm + width_cwm + height_cwm + curl_cwm, data = mean_scorch_ht) # multiple regression model
summary(scorch_mult) # display results of mult. regression

# crown scorch %
crscpct_mult <- lm(mean_crsc_pct ~ sla_cwm + ll_cwm + width_cwm + height_cwm + curl_cwm, data = mean_crsc_pct) # multiple regression model
summary(crscpct_mult) # display results of mult. regression
...
# mult regression again, without curl and height
char_mult2 <- lm(mean_char_ht ~ sla_cwm + ll_cwm + width_cwm, data = mean_char_ht) # multiple regression model
summary(char_mult2) # display results of mult. regression
```
