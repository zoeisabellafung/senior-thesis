...
thomas1 <- read.csv("data/Nicole/thomas/TF_postfire_2018.csv") %>%
  ...
  slice_head(n = 67) %>%
  as.data.frame() %>%
  mutate(plot_id = rep("NA", length = 67)) %>% # need to replace these values with actual plot ID
...
pre_fire_whole <- rbind(
  laguna_pre, bluffmesa_pre
) %>%
  mutate(category = rep("pre", length = 348))

# add thomas fire data
prefire_whole <- rbind(
  pre_fire_whole, thomas_pre
)
```
