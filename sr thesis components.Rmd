...
USDA1_postfire <- read.csv("data/USDA_RDA_pre_post_trees.csv") %>%
  ...
  filter(yr_fire_name %in% USDAfires_clean$yr_fire_name) %>%
  select(yr_fire_name, species, dataset, times_burned, tree_num, genus, species_name, hcb_post, cr_post, csh_m, cls_m, clk_m, cls_percent, clk_percent, cvs_percent, cvk_percent, cbs, bcha_m, bchm_m, bch_percent, bca, ckr, gca, gir_percent, dmr) # select only columns relating to post-fire measurements
