...
fuels_arrange <- data.frame(plot_id, fuel_class, value) %>%
  ...
  filter(!fuel_class=="total_all") %>%
  filter(!fuel_class=="total_wood")

# filter fuels data frame to only include fuels at laguna
lm_fuels <- fuels_arrange %>%
  filter(location=="Laguna Mountain")

lm_fuels_plot <- ggplot(lm_fuels, aes(fill=fuel_class, y=value, x=plot_id)) + 
...
bm_fuels <- fuels_arrange %>%
  filter(location=="Bluff Mesa")

bm_fuels_plot <- ggplot(bm_fuels, aes(fill=fuel_class, y=value, x=plot_id)) + 
  geom_bar(position="stack", stat="identity") +
...
```{r}
# SLA v. char height
lm_sla_char = lm(mean_char_ht~sla_cwm, data = mean_char_ht) # create the linear regression
