...
lm_fuelsummary <- data.frame(
  category = c("fine fuels", "1 hr", "10 hr", "100 hr", "duff/litter", "distance to crown"), 
  value = c(fine_fuels, hr_1, hr_10, hr_100, duff_litter, dist_crown)
)

# fuels summary bar plot ### FIX THIS TO MAKE MORE SENSE/BE AESTHETIC
ggplot(lm_fuelsummary, aes(x=category, y=value)) + 
  geom_bar(stat = "identity")

## NEED TITLE AND AXES LABELS
# fine fuels per plot
ggplot(lm_fuels, aes(x=plot_id, y=fine_fuels)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")

# 1 HR fuels per plot
ggplot(lm_fuels, aes(x=plot_id, y=hr_1)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")

# 10 HR fuels per plot
ggplot(lm_fuels, aes(x=plot_id, y=hr_10)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")

# 100 HR fuels per plot
ggplot(lm_fuels, aes(x=plot_id, y=hr_100)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")

# duff & litter per plot
ggplot(lm_fuels, aes(x=plot_id, y=duff_litter)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")

# distance to crown per plot
ggplot(lm_fuels, aes(x=plot_id, y=distance_to_crown_ft)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity") # missing values --> no data for that plot

# total fuels per plot
ggplot(lm_fuels, aes(x=plot_id, y=total)) + 
  geom_bar(color = "darkorchid", fill = "darkorchid", stat = "identity")
```
