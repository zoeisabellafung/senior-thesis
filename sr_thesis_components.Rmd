...
bm_fuels <- read.csv("data/Nicole/bluffmesa/BM_coordinates.csv") %>%
  ...
  mutate(litter = na_if(litter, "NA"), duff = na_if(duff, "NA"), fine_fuels = na_if(fine_fuels, "NA"))
...
# bar plot of plot x community weighted means
## laguna mountain
laguna_sla_box <-ggplot(l_abundance, aes(x = reorder(plot_id, sla_cwm), y = sla_cwm)) + 
...
## bluff mesa
bluffmesa_sla_box <-ggplot(bm_abundance, aes(x = reorder(plot_id, sla_cwm), y = sla_cwm)) + 
...
## thomas fire
thomas_sla_box <-ggplot(tf_abundance, aes(x = reorder(plot_id, sla_cwm), y = sla_cwm)) + 
...
laguna_ll_pije <- l_abundance %>%
  filter(species == "PIJE") %>%
  mutate(ll_cwm = abundance*19.666667)

laguna_ll_quke <- l_abundance %>%
  filter(species == "QUKE") %>%
  mutate(ll_cwm = abundance*13.000000)

laguna_ll_cwm <- c(laguna_ll_pije$ll_cwm, laguna_ll_quke$ll_cwm)

l_abundance <- l_abundance %>%
  mutate(ll_cwm = laguna_ll_cwm) %>%
  mutate(plot_id = as.character(plot_id))
...
bm_ll_abco <- bm_abundance %>%
  filter(species == "ABCO") %>%
  mutate(ll_cwm = abundance*3.000000)

bm_ll_cwm <- c(bm_ll_pije$ll_cwm, bm_ll_abco$ll_cwm)

bm_abundance <- bm_abundance %>%
  mutate(ll_cwm = bm_ll_cwm) %>%
  mutate(plot_id = as.character(plot_id))

## thomas fire
tf_ll_pipo <- tf_abundance %>%
  filter(species == "PIPO") %>%
  mutate(ll_cwm = abundance*22.000000)

tf_ll_quch <- tf_abundance %>%
  filter(species == "QUCH") %>%
  mutate(ll_cwm = abundance*6.166667)

tf_ll_cwm <- c(tf_ll_pipo$ll_cwm, tf_ll_quch$ll_cwm)

tf_abundance <- tf_abundance %>%
  filter(species == "PIPO" | species == "QUCH") %>%
  mutate(ll_cwm = tf_ll_cwm) %>%
  mutate(plot_id = as.character(plot_id))
```
...
# bar plot of plot x community weighted means
## laguna mountain
laguna_ll_box <-ggplot(l_abundance, aes(x = reorder(plot_id, ll_cwm), y = ll_cwm)) + 
  geom_col(fill = "deepskyblue") +
  xlab("Plot number") +
...
## bluff mesa
bluffmesa_ll_box <-ggplot(bm_abundance, aes(x = reorder(plot_id, ll_cwm), y = ll_cwm)) + 
  geom_col(fill = "slategray3") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf length (cm)") +
  theme_bw() 
...
## thomas fire
thomas_ll_box <-ggplot(tf_abundance, aes(x = reorder(plot_id, ll_cwm), y = ll_cwm)) + 
  geom_col(fill = "slategray") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf length (cm)") +
  theme_bw() 
...
laguna_width_pije <- l_abundance %>%
  filter(species == "PIJE") %>%
  mutate(width_cwm = abundance*0.1750000)

laguna_width_quke <- l_abundance %>%
  filter(species == "QUKE") %>%
  mutate(width_cwm = abundance*9.0000000)

laguna_width_cwm <- c(laguna_width_pije$width_cwm, laguna_width_quke$width_cwm)

l_abundance <- l_abundance %>%
  mutate(width_cwm = laguna_width_cwm) %>%
  mutate(plot_id = as.character(plot_id))
...
bm_width_abco <- bm_abundance %>%
  filter(species == "ABCO") %>%
  mutate(width_cwm = abundance*0.2076364)

bm_width_cwm <- c(bm_width_pije$width_cwm, bm_width_abco$width_cwm)

bm_abundance <- bm_abundance %>%
  mutate(width_cwm = bm_width_cwm) %>%
  mutate(plot_id = as.character(plot_id))

## thomas fire
tf_width_pipo <- tf_abundance %>%
  filter(species == "PIPO") %>%
  mutate(width_cwm = abundance*0.2814400)

tf_width_quch <- tf_abundance %>%
  filter(species == "QUCH") %>%
  mutate(width_cwm = abundance*3.0222222)

tf_width_cwm <- c(tf_width_pipo$width_cwm, tf_width_quch$width_cwm)

tf_abundance <- tf_abundance %>%
  filter(species == "PIPO" | species == "QUCH") %>%
  mutate(width_cwm = tf_width_cwm) %>%
  mutate(plot_id = as.character(plot_id))
```
...
# bar plot of plot x community weighted means
## laguna mountain
laguna_width_box <-ggplot(l_abundance, aes(x = reorder(plot_id, width_cwm), y = width_cwm)) + 
  geom_col(fill = "deepskyblue") +
  xlab("Plot number") +
...
## bluff mesa
bluffmesa_width_box <-ggplot(bm_abundance, aes(x = reorder(plot_id, width_cwm), y = width_cwm)) + 
  geom_col(fill = "slategray3") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf width (cm)") +
  theme_bw() 
...
## thomas fire
thomas_width_box <-ggplot(tf_abundance, aes(x = reorder(plot_id, width_cwm), y = width_cwm)) + 
  geom_col(fill = "slategray") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf width (cm)") +
  theme_bw() 
...
### plant height vegetative (in progress)
```{r}
# read in data from TRY database
...
laguna_height_pije <- l_abundance %>%
  filter(species == "PIJE") %>%
  mutate(height_cwm = abundance*50.195111)

laguna_height_quke <- l_abundance %>%
  filter(species == "QUKE") %>%
  mutate(height_cwm = abundance*27.307657)

laguna_height_cwm <- c(laguna_height_pije$height_cwm, laguna_height_quke$height_cwm)

l_abundance <- l_abundance %>%
  mutate(height_cwm = laguna_height_cwm) %>%
  mutate(plot_id = as.character(plot_id))
...
bm_height_abco <- bm_abundance %>%
  filter(species == "ABCO") %>%
  mutate(height_cwm = abundance*46.662545) # NOT ACCURATE

bm_height_cwm <- c(bm_height_pije$height_cwm, bm_height_abco$height_cwm)

bm_abundance <- bm_abundance %>%
  mutate(height_cwm = bm_height_cwm) %>%
  mutate(plot_id = as.character(plot_id))

## thomas fire
tf_height_pipo <- tf_abundance %>%
  filter(species == "PIPO") %>%
  mutate(height_cwm = abundance*18.489103)

tf_height_quch <- tf_abundance %>%
  filter(species == "QUCH") %>%
  mutate(height_cwm = abundance*2.125775)

tf_height_cwm <- c(tf_height_pipo$height_cwm, tf_height_quch$height_cwm)

tf_abundance <- tf_abundance %>%
  filter(species == "PIPO" | species == "QUCH") %>%
  mutate(height_cwm = tf_height_cwm) %>%
  mutate(plot_id = as.character(plot_id))
```
...
# bar plot of plot x community weighted means
## laguna mountain
laguna_height_box <-ggplot(l_abundance, aes(x = reorder(plot_id, height_cwm), y = height_cwm)) + 
  geom_col(fill = "deepskyblue") +
  xlab("Plot number") +
...
## bluff mesa
bluffmesa_height_box <-ggplot(bm_abundance, aes(x = reorder(plot_id, height_cwm), y = height_cwm)) + 
  geom_col(fill = "slategray3") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf height (cm)") +
  theme_bw() 

bluffmesa_height_box

## thomas fire
thomas_height_box <-ggplot(tf_abundance, aes(x = reorder(plot_id, height_cwm), y = height_cwm)) + 
  geom_col(fill = "slategray") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf height (cm)") +
  theme_bw() 
...
laguna_curl_pije <- l_abundance %>%
  filter(species == "PIJE") %>%
  mutate(curl_cwm = rep(0, length = l_abundance))

laguna_curl_quke <- l_abundance %>%
  filter(species == "QUKE") %>%
  mutate(curl_cwm = abundance*33.83)

laguna_curl_cwm <- c(laguna_curl_pije$width_cwm, laguna_curl_quke$width_cwm)

l_abundance <- l_abundance %>%
  mutate(curl_cwm = laguna_curl_cwm) %>%
  mutate(plot_id = as.character(plot_id))

## bluff mesa
bm_curl_pije <- bm_abundance %>%
  filter(species == "PIJE") %>%
  mutate(curl_cwm = rep(0, length = bm_abundance))

bm_curl_abco <- bm_abundance %>%
  filter(species == "ABCO") %>%
  mutate(curl_cwm = rep(0, length = bm_abundance))

bm_curl_cwm <- c(bm_curl_pije$curl_cwm, bm_curl_abco$curl_cwm)

bm_abundance <- bm_abundance %>%
  mutate(curl_cwm = bm_curl_cwm) %>%
  mutate(plot_id = as.character(plot_id))

## thomas fire
tf_curl_pipo <- tf_abundance %>%
  filter(species == "PIPO") %>%
  mutate(curl_cwm = rep(0, length = tf_abundance))

tf_curl_quch <- tf_abundance %>%
  filter(species == "QUCH") %>%
  mutate(curl_cwm = abundance*7.33)

tf_curl_cwm <- c(tf_curl_pipo$curl_cwm, tf_curl_quch$curl_cwm)

tf_abundance <- tf_abundance %>%
  filter(species == "PIPO" | species == "QUCH") %>%
  mutate(curl_cwm = tf_curl_cwm) %>%
  mutate(plot_id = as.character(plot_id))
```
#### building bar plots
```{r}
# bar plot of plot x community weighted means
## laguna mountain
laguna_curl_box <-ggplot(l_abundance, aes(x = reorder(plot_id, curl_cwm), y = curl_cwm)) + 
  geom_col(fill = "deepskyblue") +
  xlab("Plot number") +
...
## bluff mesa
bluffmesa_curl_box <-ggplot(bm_abundance, aes(x = reorder(plot_id, curl_cwm), y = curl_cwm)) + 
  geom_col(fill = "slategray3") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf curl (mm)") +
  theme_bw() 
...
## thomas fire
thomas_curl_box <-ggplot(tf_abundance, aes(x = reorder(plot_id, curl_cwm), y = curl_cwm)) + 
  geom_col(fill = "slategray") +
  xlab("Plot number") +
  ylab("Community weighted mean leaf curl (mm)") +
  theme_bw() 
